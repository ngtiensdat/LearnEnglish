...
export default function QuizPage() {
  ...
  const handleSubmit = () => {
    ...
    setScore(correct);
    setShowResult(true);
    toast.success(`Hoàn thành! Điểm của bạn: ${correct}/${questions.length}`);
      ...
      <div className="space-y-8 py-4">
        {!selectedTopic ? (
          <>
            <div className="text-center space-y-2">
              <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight bg-gradient-to-r from-orange-400 via-red-400 to-pink-400 bg-clip-text text-transparent">
                Luyện Tập Từ Vựng Quiz
              </h1>
              <p className="text-slate-400 max-w-xl mx-auto">
                Chọn một chủ đề và trả lời các câu hỏi nhanh để kiểm tra trí nhớ từ vựng của bạn.
              </p>
            </div>
              ...
              {QUIZ_TOPICS.map((topic, index) => (
                ...
                >
                  <div className="space-y-4 text-center">
                    ...
                    <p className="text-sm text-slate-400">{topic.questions.length} câu hỏi trắc nghiệm</p>
                  </div>
                </motion.div>
              ))}
            </div>
          ...
          <div className="space-y-6 max-w-2xl mx-auto">
            ...
            >
              <ArrowLeft className="w-4 h-4" /> Chọn chủ đề khác
            </button>

            <div className="text-center card-glass p-6">
              <h2 className="text-2xl font-bold">{selectedTopic.title}</h2>
              <p className="text-xs text-slate-400 mt-1">{selectedTopic.questions.length} câu hỏi</p>
            </div>
