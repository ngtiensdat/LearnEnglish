...
export default function RoomVocabPage() {
  ...
  useEffect(() => {
    ...
    const fetchVocab = async () => {
      try {
        ...
        console.error(err);
        toast.error(t('learnRoom.toastError'));
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleNext = () => {
    ...
    if (currentIndex < vocabList.length - 1) {
      ...
      // Finished all cards!
      toast.success(t('learnRoom.toastSuccess'));
      logProgress();
    }
  };
  ...
  if (loading) {
    return <div className="p-8 text-center text-slate-400">{language === 'vi' ? 'Đang tải từ vựng...' : 'Loading vocabulary...'}</div>;
  }
  ...
  if (vocabList.length === 0) {
    return (
      <div className="max-w-md mx-auto text-center py-20 space-y-4">
        ...
        <h2 className="text-2xl font-bold text-white">{t('learnRoom.noVocabTitle')}</h2>
        <p className="text-slate-400">{t('learnRoom.noVocabDesc')}</p>
        <button
  ...
  return (
    <>
      ...
      <div className="max-w-2xl mx-auto space-y-6 py-4">
        ...
        >
          <ArrowLeft className="w-4 h-4" /> {t('learnRoom.backBtn')}
        </button>

        <div className="text-center">
          <span className="text-sm font-semibold text-blue-400">
            {t('learnRoom.wordCounter')} {currentIndex + 1} / {vocabList.length}
          </span>
        ...
        <div className="perspective-[1000px] h-[350px] w-full cursor-pointer" onClick={() => setIsFlipped(!isFlipped)}>
          ...
          >
            ...
            <div className="absolute inset-0 w-full h-full backface-hidden card-glass p-8 flex flex-col justify-between items-center text-center">
              <span className="text-xs font-semibold text-slate-500 tracking-wider">{t('learnRoom.flipFront')}</span>
              ...
              <div className="space-y-4">
                ...
                <button
                  ...
                  title={t('learnRoom.pronounceTitle')}
                >
                  <Volume2 className="w-5 h-5" />
                </button>
              </div>

              <span className="text-xs text-slate-400 italic">{t('learnRoom.flipFrontHelper')}</span>
            </div>
            ...
            <div className="absolute inset-0 w-full h-full backface-hidden rotate-y-180 card-glass p-8 flex flex-col justify-between items-center text-center bg-gradient-to-br from-blue-950/40 to-slate-900/40">
              <span className="text-xs font-semibold text-blue-400 tracking-wider">{t('learnRoom.flipBack')}</span>
              ...
              <div className="space-y-4">
                ...
                {currentVocab.example && (
                  <div className="p-3 bg-slate-950/60 rounded-xl max-w-md border border-slate-800/80">
                    <p className="text-xs text-slate-400">{language === 'vi' ? 'Ví dụ:' : 'Example:'}</p>
                    <p className="text-sm text-slate-200 mt-1 italic">{currentVocab.example}</p>
                  </div>
                )}
              </div>

              <span className="text-xs text-slate-400 italic">{t('learnRoom.flipBackHelper')}</span>
            </div>
          </motion.div>
        </div>
        ...
        <div className="flex gap-4">
          ...
          >
            {currentIndex === vocabList.length - 1 ? t('learnRoom.finishBtn') : t('learnRoom.nextBtn')}
          </button>
        </div>
      </div>
    </>
