---
description: Hướng dẫn phân quyền vai trò người dùng (RBAC) trong dự án FOOD AI
---
...
Các vai trò được định nghĩa thông qua enum `UserRole` trong [schema.prisma](file:///e:/FOOD_AI_code/my-web/backend/prisma/schema.prisma):

```prisma
enum UserRole {
  CUSTOMER     // Khách hàng / Thực khách
  RESTAURANT   // Chủ nhà hàng / Thương gia
...
@Controller('restaurant-admin')
@UseGuards(JwtAuthGuard, RolesGuard)
@Roles(UserRole.RESTAURANT, UserRole.ADMIN) // Chỉ RESTAURANT và ADMIN được phép truy cập
...
[RolesGuard.ts](file:///e:/FOOD_AI_code/my-web/backend/src/common/guards/roles.guard.ts) sẽ sử dụng `Reflector` để đọc metadata vai trò cần có và đối chiếu với `request.user.role` được giải mã từ JWT:
...
import { UserRole } from '@/types/user'; // Hoặc string literal 'CUSTOMER' | 'RESTAURANT' | 'ADMIN'

const MyComponent = () => {
  const { user, isCustomer, isAdmin, isRestaurant } = useAuth();

  return (
    <div>
      {/* Hiển thị UI có điều kiện */}
      {isRestaurant && <p>Chào mừng Đối tác Nhà hàng!</p>}
      {isAdmin && <button>Vào trang Quản trị viên</button>}
      
      {/* Kiểm tra trực tiếp bằng user.role */}
      {user?.role === 'CUSTOMER' && <p>Bạn đang là Thực khách</p>}
    </div>
  );
};
```
...
const handleRedirect = (role: string) => {
  if (role === 'ADMIN') {
    ...
    router.push('/dashboard'); // Thực khách mặc định
  }
};
```