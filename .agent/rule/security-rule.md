...
- [ ] All inputs are validated via DTOs (class-validator).
- [ ] No magic strings for roles (Use Enums from Prisma).
- [ ] Response objects are filtered (No passwords/tokens).
- [ ] Rate limits are applied to AI/Auth routes (Cost & Spam protection).
- [ ] Secrets are managed via `@nestjs/config` and `.env`.
- [ ] No external entities parsed in XML (disable DTDs).
- [ ] CORS is properly configured for production.
- [ ] **OWNERSHIP CHECK:** Verified that the user owns the resource they are modifying (IDOR protection).
- [ ] **AI PROMPT SAFETY:** Input sent to LLM is sanitized to prevent prompt injection.
